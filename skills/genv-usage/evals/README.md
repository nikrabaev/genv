# genv-usage skill — eval harness

Reproducible benchmark for the `genv-usage` skill. Each eval is a realistic task a
developer would type in a genv-managed repo. Runs compare **with-skill** vs a
**baseline** (no skill) to measure what the skill actually changes.

## Fixture

A throwaway monorepo, `genv init --no-encrypt`'d so every vault is **plaintext and
git-ignored** — the suite runs fully headless with no passphrase or keychain. (For
an encrypted vault instead, auth resolves from `GENV_VAULT_AUTH_<VAULT>` or
`.genv/auth.local.json`; off a TTY a missing key is a hard error.)

**Vaults** (each `genv-local`, plaintext):

- `local` — the default vault (`defaults.vault`), the everyday dev values.
- `prod` — a second vault (`vault add prod --type genv-local --config
  filename=.genv/prod.json,encryption=false`) — the prod generation context for
  eval 3.
...
invoke genv non-interactively, expose `genv` on PATH (or `go run ./cmd/genv` from
the repo); no auth wrapper is needed since the vaults are plaintext.
...
4–6 are graded with a **durability** check: re-run `genv generate` and confirm the
...
secret is passed as a plaintext CLI argument — `genv set STRIPE_SECRET_KEY
sk_test_… ` — leaking it into shell history; with the skill it is piped on stdin
(`printf '%s' "$KEY" | genv set STRIPE_SECRET_KEY`). Both runs store a round-trip
value and flag genv's single-line limit on eval 6, but they differ in fidelity: the
skill folds the PEM to one line with escaped `\n` (which `genv generate` renders as
...
(probing `genv --help`, reading `genv.json`, inferring the model) — and a
**guardrail** that keeps secrets off the command line. Both effects grow with weaker
agents or more obscure genv behavior.
...
reverse-engineer — evals 4–6, where it reads `--help` and genv's source before
